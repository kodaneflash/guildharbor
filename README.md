...
- Settings → Security changes passwords with Better Auth and invalidates other sessions as part of password change. TOTP enrollment shows a setup key, then requires a valid authenticator code before enabling 2FA. Subsequent password logins require TOTP. Members can inspect and revoke sessions, revoke other sessions, replace recovery codes after password confirmation and disable TOTP after password confirmation. Email OTP sign-in is disabled because it is not an enabled product flow and the installed TOTP plugin does not guard that route. Social sign-in continues to rely on the social provider's authentication protections.
...
Omitting the variable defaults to **public**; any other value fails environment validation. The example environment uses public mode. This setting never makes community content accessible to guests.
