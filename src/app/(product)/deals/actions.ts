...
export async function draftAction(_: { message: string }, form: FormData) {
  ...
  const result = await saveDealDraft(raw); if (result.error) return { message: result.error }; redirect(`/deals/${result.id}`);
}
export async function dealAction(_: { message: string }, form: FormData) {
  const result = await transitionDeal(Object.fromEntries(form)); revalidatePath("/deals", "layout"); return { message: result.error ?? `Agreement is now ${result.state}.` };
}
