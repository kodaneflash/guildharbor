...
const typeStyles: Record<ThreadType, string> = {
  ...
  selling: "border-selling/40 bg-selling text-white",
  buying: "border-sticky/40 bg-sticky text-page-deep",
  service: "border-orange/40 bg-orange text-page-deep",
...
const badgeStyles: Record<ThreadBadge, string> = {
  ...
  "Top seller": "border-yellow/35 bg-yellow/15 text-yellow",
...
export function ThreadTypeBadge({ type }: { type: ThreadType }) {
  return (
    <span className={cn("inline-flex h-6 items-center rounded-[5px] border px-1.5 text-[11px] font-extrabold capitalize leading-none", typeStyles[type])}>
      {type}
    </span>
  );
}
...
export function ThreadFlagBadge({ badge }: { badge: ThreadBadge }) {
  return (
    <span className={cn("inline-flex h-6 items-center gap-1 rounded-[5px] border px-1.5 text-[11px] font-extrabold uppercase leading-none", badgeStyles[badge])}>
