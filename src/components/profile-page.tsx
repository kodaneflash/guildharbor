...
export function ProfilePage({ profile, viewerId }: { profile: ProfileSummary; viewerId: string }) {
  return (
        ...
        <div className="relative flex min-h-[230px] flex-col justify-end gap-5 p-5 sm:flex-row sm:items-end sm:justify-between sm:p-8">
          <div className="flex items-end gap-4 sm:gap-6">
            ...
            <div className="min-w-0 pb-2">
              ...
              <p className="mt-2 text-body-sm font-semibold text-text-secondary">
                {profile.tagline}
              </p>
              {profile.discord && <p className="mt-1 text-body-xs text-category">Discord: {profile.discord}</p>}
              {profile.telegram && (
                <p className="mt-1 text-body-xs text-category">{profile.telegram}</p>
              )}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
            {profile.uid === viewerId ? (
              <Link className="button-secondary col-span-2" href="/settings/profile">
                Edit your profile
              </Link>
            ) : (
              <Link className="button-secondary col-span-2" href={`/messages?to=${encodeURIComponent(profile.username)}`}>
                <Send className="size-4" aria-hidden="true" /> Message {profile.username}
              </Link>
            )}
          </div>
        </div>
  ...
  return (
    <div className="grid items-start gap-5 lg:grid-cols-[300px_minmax(0,1fr)_310px]">
      <aside className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <StatCard
            ...
            label="Visible community reputation"
            accent
          />
          <StatCard
            value={profile.vouches.positive.toLocaleString()}
            label="Visible community vouches"
            accent
          />
        </div>
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-2 overflow-hidden rounded-md border border-border bg-border sm:grid-cols-2">
          <StatCard
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {profile.groups.map((group) => (
              ...
              >
                ...
                <p className="mt-3 text-body-xs leading-5 text-text-muted">
                  {group.subtitle}
                </p>
              </div>
            ))}
      ...
      <aside className="space-y-4">
        <div className="surface flex items-center gap-3 p-5">
          ...
          <div>
            <strong className="block text-body-sm uppercase text-text">
              Years of service
            </strong>
        ...
        <div className="surface flex items-center gap-3 p-5">
          ...
          <strong className="text-body-sm uppercase text-text">
            Email verified
          </strong>
        </div>
        <section className="surface overflow-hidden">
          ...
          {!profile.activity.length && (
            <p className="p-5 text-body-sm text-text-muted">No recent activity.</p>
          )}
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 block text-body-xs font-semibold text-text-muted">
        {label}
      </span>
    </div>
  );
...
function InfoRow({
  ...
  return (
    <div className="surface flex min-h-14 items-center gap-3 px-4">
      ...
      <span className="text-body-xs font-semibold text-text-muted">{label}</span>
      <strong className="ml-auto text-right text-body-xs text-text-secondary">
        {value}
      </strong>
    </div>
  );
